...
1. On the Session Monitoring page, additional icons are presented based on what extra metadata is present for that session, such as keystroke data for both RDP and SSH, and process data for RDP.
...
1. Save the file as `gsresvc.mst` in the column you extracted the installer into. This transform file now contains your customizations for the ServiceInstall Arguments.
1. Close Orca.
...
1. The Thycotic session recording agent should be visible in the Installed Applications section.
